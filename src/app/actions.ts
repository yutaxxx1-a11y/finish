...
export async function finishPostAction(_: FinishActionState, formData: FormData): Promise<FinishActionState> {
  ...
  try {
    ...
    const { data: currentDna } = await db.from("taste_dna").select("*").eq("user_id", user.id).maybeSingle();
    const nextDna = mergeDna(dnaFromRow(currentDna), update.delta);

    await db.from("taste_dna").upsert({
      user_id: user.id,
      ...nextDna,
...
async function toggleRelation(table: "likes" | "bookmarks", postId: string) {
  ...
  const { data: existing } = await db.from(table).select("id").eq("user_id", user.id).eq("post_id", postId).maybeSingle();
...
export async function toggleTastePublicAction(publicValue: boolean) {
  ...
  await db.from("profiles").update({ taste_profile_public: publicValue }).eq("id", user.id);
